#!/usr/bin/env python3
"""
Backfill per-side credits into open Iron Condor rows.

IC rows are logged with all four strikes in Notes but, until now, without any
record of what each vertical was opened for. That split is needed the moment
one side is bought back on its own: the surviving vertical has to carry its own
share of the opening credit out to its own row, and it can't be re-derived from
the closing fills.

New ICs record the split at open time. This backfills the rows that predate
that by re-fetching each row's opening order from Tastytrade and re-deriving
the split from the original legs. Rows whose opening order can't be found are
...
def main():
    ...
    for i, row in enumerate(logger.sheet.get_all_values(), start=1):
        if len(row) < 16 or row[3] != 'IC' or row[4] != 'Open':
            continue
        if logger._parse_ic_sides(row[15]):
            continue  # already recorded
        targets.append((i, row))

    if not targets:
        print('✓ Every open IC row already records its per-side credits')
        return 0
    ...
    for row_num, row in targets:
        ...
        notes = logger._ic_notes(match['strikes'], match['side_breakdown'])
        print(f"  → {notes}")
