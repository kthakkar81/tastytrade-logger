...
print(f"\nSearching {len(all_rows)} rows for 5/28...\n")

for i, row in enumerate(all_rows, start=1):
    opening_date = row[0] if len(row) > 0 else ''
    if '5/28' in opening_date:
        ...
        print(f"  {underlying} {strategy} - {status}")
        print(f"  Qty: {qty}, Price: {price}, Fees: {fees}")
        print()
